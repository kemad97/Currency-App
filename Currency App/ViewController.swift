...
class ViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            if indexPath.row < sortedCurrencies.count {
                let currencyCode = sortedCurrencies[indexPath.row]
                if let rate = rates[currencyCode],
                   let currency = Currency(rawValue: currencyCode) {
                    cell.textLabel?.text = "1 \(selectedBaseCurrency.symbol) = \(rate) \(currency.symbol) (\(currencyCode))"
                } else {
                    // Fallback for currencies not in our enum
                    cell.textLabel?.text = "\(currencyCode): \(rates[currencyCode] ?? 0)"
                }
            }
